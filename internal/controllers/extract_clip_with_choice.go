...
// @Param   		t				    query	string	false	"t"		                    minlength(1)  	maxlength(100)
// @Produce      	json
// @Success      	200  {object}   models.JsonDTOListRsp[models.ListVideoDownLoad]
// @Router       	/video/downloadchoice [get]
// @Security		BearerAuth
func ExtractClipWithChoice(c *gin.Context) {
	...
	for {
		select {
		case <-timeout:
			fmt.Println("\t\t> error, waiting for command result timed out: ", cmd_ExtractClip)
			jsonRspDTOCamerasBasicInfos.Message = "No response from the device, timeout: " + cmd_ExtractClip
		...
		case <-ticker.C:
			if storedMsg, ok := messageMap.Load(RequestUUID); ok {
				...
				video, err := minioclient.GetPresignedURL(msg.PayLoad.VideoDownLoad.StorageBucket, msg.PayLoad.VideoDownLoad.Video)
				if err != nil {
					jsonRspDTOCamerasBasicInfos.Code = http.StatusNotFound
					jsonRspDTOCamerasBasicInfos.Message = "Video not found: " + err.Error()
					c.JSON(http.StatusNotFound, &jsonRspDTOCamerasBasicInfos)
					return
				}
...
func cutVideo(inputFile, outputFile string, startTime, endTime time.Time) error {
	startSeconds := startTime.Sub(time.Date(1, 1, 1, 0, 0, 0, 0, time.UTC)).Seconds()
	endSeconds := endTime.Sub(time.Date(1, 1, 1, 0, 0, 0, 0, time.UTC)).Seconds()

	startStr := strconv.FormatFloat(startSeconds, 'f', 2, 64)
	durationStr := strconv.FormatFloat(endSeconds-startSeconds, 'f', 2, 64)

	cmd := exec.Command("ffmpeg", "-i", inputFile, "-ss", startStr, "-t", durationStr, "-c", "copy", outputFile)
	err := cmd.Run()
	if err != nil {
		return fmt.Errorf("error extracting video segment: %w", err)
	}
	return nil
}

func downloadVideo(videoURL, outputPath string) error {
	// Use your method to download the file from Minio
